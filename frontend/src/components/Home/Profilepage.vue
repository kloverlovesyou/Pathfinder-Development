...
const userName = ref("");
const activities = ref([]); // ✅ unified data holder
...
onMounted(async () => {
  ...
  if (savedUser) {
    ...
    // ✅ Fetch the activities if applicantID exists
    if (user.applicantID) {
      try {
...
async function fetchMyActivities() {
  ...
  try {
    ...
    // ✅ Make sure to access .activities
...
// ✅ Handle attendance submission (for trainings)
...
// ✅ Open modal with selected activity
